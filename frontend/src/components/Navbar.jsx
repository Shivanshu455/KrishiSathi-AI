...
function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 bg-white/90 backdrop-blur-lg border-b border-green-100">
      {/* Responsive padding on both sides gives equal breathing room to both the logo and the logout button */}
      <div className="flex h-20 w-full items-center justify-between px-8 md:px-12 lg:px-20">
        ...
        >
          ...
          <div>
            <h1 className="text-2xl xl:text-3xl font-extrabold leading-none">
              <span className="text-gray-900">Krishi</span>
        ...
        <nav className="hidden lg:flex items-center gap-6 xl:gap-8 justify-center">
          ...
          {isLoggedIn && (
            <>
              <NavLink to="/crop-planner" className={navItem}>
                Crop Planner
              </NavLink>
      ...
      {mobileOpen && (
        <div className="lg:hidden border-t border-green-100 bg-white px-8 py-4 space-y-3 shadow-lg">
          ...
          {isLoggedIn && (
            <>
              <NavLink to="/crop-planner" className={navItem} onClick={() => setMobileOpen(false)}>
                Crop Planner
              </NavLink>

              <NavLink to="/analysis" className={navItem} onClick={() => setMobileOpen(false)}>
                Farm Analysis
              </NavLink>
          ...
          <div className="pt-3 flex items-center justify-between border-t border-gray-100">
            ...
            {!isLoggedIn ? (
              <div className="flex gap-3">
                <NavLink to="/login" className="px-4 py-2 rounded-xl border border-green-600 text-green-700 font-semibold text-sm">
                  Login
                </NavLink>
                <NavLink to="/register" className="px-4 py-2 rounded-xl bg-green-600 text-white font-semibold text-sm">
                  Register
                </NavLink>
              </div>
            ) : (
              <button onClick={logout} className="px-4 py-2 rounded-xl border border-red-500 text-red-500 font-semibold text-sm">
                Logout
              </button>
            )}
          </div>
